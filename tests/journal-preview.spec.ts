...
test('journal days show three representative photos before expanding', async ({ page }) => {
  ...
  if (test.info().project.name === 'desktop') {
    ...
    await expect(page.locator(".quickAlbum .frontAlbumBase").first()).toHaveAttribute("src", /album-closed-ivory/);
...
test('album reader preserves order and supports list, scrubber and nested photo navigation', async ({ page }) => {
  ...
  await reader.getByRole('slider', { name: '앨범 책장 이동' }).fill('3');
  await expect(reader.locator('.albumPagerActions p')).toHaveText('3 / 3 펼침');
...
for (const entry of ['saved albums', 'journal shortcut']) {
  test(`${entry} reflects photo edits without moving the current spread`, async ({ page, isMobile }) => {
    ...
    await reader.getByRole('button', { name: 'record-5.jpg 상세보기' }).click();
    ...
    await detail.getByPlaceholder('내용 입력').fill('바람이 좋았던 날');
    await detail.getByRole('button', { name: '확인', exact: true }).click();
    await detail.getByTitle('닫기', { exact: true }).click();

    await expect(reader.locator('.albumPagerActions p')).toHaveText('2 / 3 펼침');
    await expect(reader.locator('.albumPageCaption').first()).toContainText('바람이 좋았던 날');
    await reader.getByRole('button', { name: 'record-5.jpg 상세보기' }).click();
    ...
    await expect(detail.locator('.viewerMeta')).toContainText('2회');
    await detail.getByRole('button', { name: '정보·기록', exact: true }).click();
