...
const {
  arrangeJournalItems, filterJournalMonth, formatJournalDate, journalMonths,
  journalMonthTitle, makeAlbumPages, makeAlbumSpreads, isPortraitMedia, mediaSummary, resolveJournalMonth, shuffleAlbumItems, syncAlbumMedia,
...
test('opening an album preserves saved order, including a partly filled last spread', () => {
  ...
  const pages = makeAlbumPages(source);
  assert.deepEqual(pages.map((page) => page.length), [4, 4, 1]);
  assert.deepEqual(pages.flat(), source);
  assert.deepEqual(makeAlbumPages(source), pages);
  assert.deepEqual(makeAlbumPages([]), []);
  assert.throws(() => makeAlbumPages(source, 0), RangeError);
});
...
test('all-portrait albums pair one photo per side, including an odd final page', () => {
  const items = Array.from({ length: 5 }, (_, index) => photo(String(index), null, { width: 500, height: 900 }));
  assert.deepEqual(makeAlbumSpreads(items).map(({ left, right }) => [left.length, right.length]), [[1, 1], [1, 1], [1, 0]]);
  assert.deepEqual(makeAlbumSpreads([items[0]])[0], { left: [items[0]], right: [] });
});

test('landscape, square and unknown dimensions retain the two-photos-per-side layout', () => {
  const items = [
  ...
  assert.deepEqual(makeAlbumSpreads(items).map(({ left, right }) => [left.length, right.length]), [[2, 2], [1, 0]]);
});
...
test('updating album media leaves the current spread and last-page contents in place', () => {
  ...
  const before = makeAlbumPages(albums[0].items);
  const after = makeAlbumPages(syncAlbumMedia(albums, current)[0].items);
  assert.deepEqual(after.map((page) => page.map((item) => item.id)), before.map((page) => page.map((item) => item.id)));
  assert.equal(after[1][0].comment, '두 번째 펼침의 기록');
  assert.equal(after[2].length, 1);
});
