...
test('redesigned views fit and retain photo workflows', async ({ page }) => {
  ...
  await expect(page.locator('.albumPagePhoto[data-side="right"]').first()).toHaveCSS('opacity', '0');
  await page.waitForTimeout(120);
  await expect(page.locator('.albumPagePhoto[data-side="left"]').first()).toHaveCSS('opacity', '0');
  ...
  await expect(page.locator('.albumPagerActions p')).toHaveText('2 / 5 펼침');
  await expect(page.getByTitle('이전 책장', { exact: true })).toBeEnabled();
  await expect(page.locator('.albumPaper.left .albumPageNumber')).toHaveText('3');
  await expect(page.locator('.albumPaper.right .albumPageNumber')).toHaveText('4');
  await page.waitForTimeout(500);
  await expect(page.locator('.albumTurningSheet')).toHaveCount(0);
  await page.getByTitle('다음 책장', { exact: true }).click();
  await page.getByRole('button', { name: '다시 섞기' }).click();
  await page.waitForTimeout(900);
  await expect(page.locator('.albumPagerActions p')).toHaveText('1 / 5 펼침');
