...
test('fabric album colors survive create, edit and reload', async ({ page }) => {
  ...
  expect(spread!.width / spread!.height).toBeCloseTo(2.4, 2);
  ...
  for (const [side, ratio] of [['left', 1.65], ['right', 1.35]] as const) {
    const photos = reader.locator(`.albumPaper.${side} .albumPagePhoto`);
    const top = (await photos.nth(0).boundingBox())!;
  ...
  for (const size of sizes) {
    ...
    expect(book.width / book.height).toBeCloseTo(2.4, 2);
